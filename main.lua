--[[
    Zensai Interface Suite
    Ultra-Smooth & Enhanced Edition
    Author: dawid (Enhanced for Zensai UI)
    License: MIT
--]]
...
[2] = function(...)
local maui, script, require, getfenv, setfenv = b(2)
local Acrylic = {
...
local function createAcrylicBlur(distance)
	...
	local function render()
		if not model.Parent or model.Transparency >= 1 then
			return
		end
...
[4] = function(...)
local maui, script, require, getfenv, setfenv = b(4)
local Creator = require(script.Parent.Parent.Creator)
...
[5] = function(...)
local maui, script, require, getfenv, setfenv = b(5)
...
[6] = function(...)
local maui, script, require, getfenv, setfenv = b(6)
...
[8] = function(...)
local maui, script, require, getfenv, setfenv = b(8)
return {
...
[9] = function(...)
local maui, script, require, getfenv, setfenv = b(9)
local Root = script.Parent.Parent
...
function Dialog:Create()
	...
	function NewDialog:Close()
		...
		task.spawn(function()
			pcall(function()
				if NewDialog.Root and NewDialog.Root:FindFirstChild("UIStroke") then
...
[11] = function(...)
local maui, script, require, getfenv, setfenv = b(11)
local Root = script.Parent.Parent
local Flipper = require(Root.Packages.Flipper)
local Creator = require(Root.Creator)
local New = Creator.New
...
[12] = function(...)
local maui, script, require, getfenv, setfenv = b(12)
local Root = script.Parent.Parent
local Flipper = require(Root.Packages.Flipper)
local Creator = require(Root.Creator)
local Acrylic = require(Root.Acrylic)
...
function Notification:New(Config)
	...
	CloseConnection = NewNotification.CloseButton.MouseButton1Click:Connect(function()
...
[13] = function(...)
local maui, script, require, getfenv, setfenv = b(13)
local Root = script.Parent.Parent
local Creator = require(Root.Creator)
...
[14] = function(...)
local maui, script, require, getfenv, setfenv = b(14)
local Root = script.Parent.Parent
local Flipper = require(Root.Packages.Flipper)
local Creator = require(Root.Creator)
...
function TabModule:SelectTab(Tab)
	...
	local thisTransition = CurrentTransition

	task.spawn(function()
		Window.ContainerHolder.Parent = Window.ContainerAnim

		Window.ContainerPosMotor:setGoal(Spring(15, { frequency = 12 }))
		Window.ContainerBackMotor:setGoal(Spring(1, { frequency = 12 }))
		task.wait(0.1)
		if thisTransition ~= CurrentTransition then
			return
		end
...
[15] = function(...)
local maui, script, require, getfenv, setfenv = b(15)
...
[16] = function(...)
local maui, script, require, getfenv, setfenv = b(16)
local Root = script.Parent.Parent
...
[18] = function(...)
local maui, script, require, getfenv, setfenv = b(18)
...
[19] = function(...)
local maui, script, require, getfenv, setfenv = b(19)
local Elements = {}
...
[20] = function(...)
local maui, script, require, getfenv, setfenv = b(20)
local Root = script.Parent.Parent
local Creator = require(Root.Creator)
...
[21] = function(...)
local maui, script, require, getfenv, setfenv = b(21)
local UserInputService = game:GetService("UserInputService")
...
function Element:New(Idx, Config)
	...
	local function CreateColorDialog()
		...
		local DialogSignals = {}
		local function AddDialogSignal(Signal, Handler)
			table.insert(DialogSignals, Signal:Connect(Handler))
		end

		AddDialogSignal(HexInput.Input.FocusLost, function(Enter)
			if Enter then
		...
		AddDialogSignal(RedInput.Input.FocusLost, function(Enter)
			if Enter then
		...
		AddDialogSignal(GreenInput.Input.FocusLost, function(Enter)
			if Enter then
				local CurrentColor = GetRGB()
		...
		AddDialogSignal(BlueInput.Input.FocusLost, function(Enter)
			if Enter then
				local CurrentColor = GetRGB()
				local Success, Result =
		...
		if Config.Transparency and AlphaInput then
			AddDialogSignal(AlphaInput.Input.FocusLost, function(Enter)
				if Enter then
					pcall(function()
		...
		local SatVibDrag = false
		local HueDragState = false
		local AlphaDragState = false
		...
		local function UpdateSatVib(Pos)
			...
			local RangeY = math.max(MaxY - MinY, 1)

			Sat = (MouseX - MinX) / RangeX
			Vib = 1 - ((MouseY - MinY) / RangeY)
			Display()
		end
		...
		local function UpdateHue(Pos)
			...
			Hue = (MouseY - MinY) / RangeY
			Display()
		end

		local function UpdateAlpha(Pos)
			if not TransparencySlider then return end
			local MinY = TransparencySlider.AbsolutePosition.Y
			local MaxY = MinY + TransparencySlider.AbsoluteSize.Y
			local MouseY = math.clamp(Pos.Y, MinY, MaxY)
			local RangeY = math.max(MaxY - MinY, 1)

			Transparency = 1 - ((MouseY - MinY) / RangeY)
			Display()
		end

		AddDialogSignal(SatVibMap.InputBegan, function(Input)
			if
				Input.UserInputType == Enum.UserInputType.MouseButton1
				or Input.UserInputType == Enum.UserInputType.Touch
			then
		...
		AddDialogSignal(HueSlider.InputBegan, function(Input)
			if
				Input.UserInputType == Enum.UserInputType.MouseButton1
				or Input.UserInputType == Enum.UserInputType.Touch
			then
		...
		if Config.Transparency and TransparencySlider then
			AddDialogSignal(TransparencySlider.InputBegan, function(Input)
				if
					Input.UserInputType == Enum.UserInputType.MouseButton1
					or Input.UserInputType == Enum.UserInputType.Touch
				then
		...
		AddDialogSignal(UserInputService.InputChanged, function(Input)
			if
				Input.UserInputType == Enum.UserInputType.MouseMovement
				or Input.UserInputType == Enum.UserInputType.Touch
			then
				if SatVibDrag then
					UpdateSatVib(Input.Position)
				elseif HueDragState then
					UpdateHue(Input.Position)
				elseif AlphaDragState then
					UpdateAlpha(Input.Position)
				end
			end
		end)

		AddDialogSignal(UserInputService.InputEnded, function(Input)
			if
				Input.UserInputType == Enum.UserInputType.MouseButton1
				or Input.UserInputType == Enum.UserInputType.Touch
			then
				SatVibDrag = false
				HueDragState = false
				AlphaDragState = false
			end
		end)

		local function CleanDialogSignals()
			for _, conn in ipairs(DialogSignals) do
				pcall(function()
					conn:Disconnect()
				end)
			end
			table.clear(DialogSignals)
		end

		AddDialogSignal(Dialog.TintFrame.Destroying, CleanDialogSignals)
...
[23] = function(...)
local maui, script, require, getfenv, setfenv = b(23)
local Root = script.Parent.Parent
local Creator = require(Root.Creator)
...
[24] = function(...)
local maui, script, require, getfenv, setfenv = b(24)
local UserInputService = game:GetService("UserInputService")
		...
		elseif Keybind.Mode == "Hold" then
			...
			if Key == "MouseLeft" or Key == "MouseRight" then
				...
				local Success, KeyCode = pcall(function()
					return Enum.KeyCode[Keybind.Value]
				end)
...
[25] = function(...)
local maui, script, require, getfenv, setfenv = b(25)
local Root = script.Parent.Parent
local Components = Root.Components
local Flipper = require(Root.Packages.Flipper)
local Creator = require(Root.Creator)
...
[26] = function(...)
local maui, script, require, getfenv, setfenv = b(26)
local UserInputService = game:GetService("UserInputService")
local Root = script.Parent.Parent
local Creator = require(Root.Creator)
...
function Element:New(Idx, Config)
	...
	Creator.AddSignal(SliderInner.InputBegan, StartDrag)
	...
	function Slider:SetValue(Value)
		self.Value = Library:Round(math.clamp(tonumber(Value) or Slider.Min, Slider.Min, Slider.Max), Slider.Rounding)
		local Range = Slider.Max - Slider.Min
		if Range <= 0 then
			Range = 1
		end
		local Progress = math.clamp((self.Value - Slider.Min) / Range, 0, 1)
		SliderDot.Position = UDim2.new(Progress, -7, 0.5, 0)
...
[27] = function(...)
local maui, script, require, getfenv, setfenv = b(27)
local TweenService = game:GetService("TweenService")
local Root = script.Parent.Parent
local Creator = require(Root.Creator)
...
[28] = function(...)
local maui, script, require, getfenv, setfenv = b(28)
...
[30] = function(...)
local maui, script, require, getfenv, setfenv = b(30)
local Flipper = {
...
[31] = function(...)
local maui, script, require, getfenv, setfenv = b(31)
local RunService = game:GetService("RunService")
...
[32] = function(...)
local maui, script, require, getfenv, setfenv = b(32)
return function()
	local RunService = game:GetService("RunService")
...
[33] = function(...)
local maui, script, require, getfenv, setfenv = b(33)
local BaseMotor = require(script.Parent.BaseMotor)
...
[34] = function(...)
local maui, script, require, getfenv, setfenv = b(34)
return function()
...
[35] = function(...)
local maui, script, require, getfenv, setfenv = b(35)
local Instant = {}
...
[36] = function(...)
local maui, script, require, getfenv, setfenv = b(36)
return function()
	local Instant = require(script.Parent.Instant)
...
[37] = function(...)
local maui, script, require, getfenv, setfenv = b(37)
local Linear = {}
...
[38] = function(...)
local maui, script, require, getfenv, setfenv = b(38)
return function()
	local SingleMotor = require(script.Parent.SingleMotor)
...
[39] = function(...)
local maui, script, require, getfenv, setfenv = b(39)
local Connection = {}
...
[40] = function(...)
local maui, script, require, getfenv, setfenv = b(40)
return function()
	local Signal = require(script.Parent.Signal)
...
[41] = function(...)
local maui, script, require, getfenv, setfenv = b(41)
local BaseMotor = require(script.Parent.BaseMotor)
...
[42] = function(...)
local maui, script, require, getfenv, setfenv = b(42)
return function()
	local SingleMotor = require(script.Parent.SingleMotor)
	local Instant = require(script.Parent.Instant)
...
[43] = function(...)
local maui, script, require, getfenv, setfenv = b(43)
...
[44] = function(...)
local maui, script, require, getfenv, setfenv = b(44)
return function()
	local SingleMotor = require(script.Parent.SingleMotor)
	local Spring = require(script.Parent.Spring)
...
[45] = function(...)
local maui, script, require, getfenv, setfenv = b(45)
...
[46] = function(...)
local maui, script, require, getfenv, setfenv = b(46)
return function()
	local isMotor = require(script.Parent.isMotor)
...
[47] = function(...)
local maui, script, require, getfenv, setfenv = b(47)
local Themes = {
	Names = {
		"Dark",
		"Darker",
...
[48] = function(...)
local maui, script, require, getfenv, setfenv = b(48)
return {
	Name = "Amethyst",
...
[49] = function(...)
local maui, script, require, getfenv, setfenv = b(49)
return {
	Name = "Aqua",
...
[50] = function(...)
local maui, script, require, getfenv, setfenv = b(50)
return {
	Name = "Dark",
...
[51] = function(...)
local maui, script, require, getfenv, setfenv = b(51)
return {
	Name = "Darker",
...
[52] = function(...)
local maui, script, require, getfenv, setfenv = b(52)
return {
	Name = "Light",
...
[53] = function(...)
local maui, script, require, getfenv, setfenv = b(53)
return {
	Name = "Rose",
